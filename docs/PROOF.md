...
A real heterogeneous local consumer now completes with Advocate through the OpenAI-compatible Ollama adapter, Opposer through a separate HTTP worker process, Market Analyst through a separate MCP worker process, and the Builtin Council. The workflow reached `COMPLETE`; its composition retained all three external adapter identities and the receipt preserved the same workflow ID. This is `HETEROGENEOUS_REASONING_STACK_PASS` and local-provider evidence only. Custom external Council remains `NOT_IMPLEMENTED` because the canonical CouncilDecision contract is not yet exposed at the adapter boundary; the factory rejects that configuration rather than treating a RoleArtifact as Council output.
