...
| Binance Agentic MCP | **Agent OS tool/account/access plane** and, eventually, a bounded trade-handoff plane. It may expose tools or account context to an agent, subject to explicit adapter and authority boundaries. | **ADAPTER LOCAL_PASS / AUTH_BLOCKED_EXTERNAL.** Official endpoint identity and read-only adapter boundary are recorded in `ZO-BIN-MB8-binance-agentic-mcp-read.json`. The raw 401 is preserved as `UNAUTHENTICATED_PROBE`; supported `mcp-remote` OAuth discovery reached Binance but failed before consent because dynamic client registration is unsupported. It is not an execution writer. |
| Binance Skills Hub | **Reasoning/evidence capability plane**: official skill output may be validated, normalized, and provenance-bound into role evidence. | **ADAPTER LOCAL_PASS / PUBLIC_READ BLOCKED_EXTERNAL.** Official source `binance/binance-skills-hub` v2.0.0 and binance-cli v2.1.1 are installed/inspected; credential-free ticker/mark-price calls hit Binance eligibility restriction. No live Skills observation is claimed. Skill output is not a `TradeThesis` or an `ExecutionMandate`. |
...
- Claims about Agentic MCP and Skills Hub now use their bounded adapter/evidence receipts above; they do not establish authenticated reads, live market capability, account truth, or execution authority. Claims about x402, Agentic Wallet/Web3, or Binance AI Chat remain `PLANNED / UNVERIFIED` unless a future record cites a current official source and states the exact supported claim.
