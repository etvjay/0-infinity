...
allowed_files:
  ...
  - docs/development/SUBMISSION_CHECKLIST.md
forbidden_files:
  - credentials
  - src/testnet/index.ts
  - live writer/mainnet configuration
implementation_plan:
  - Read native public bookTicker observations over a bounded WebSocket session.
  - Preserve raw and normalized observations with exact update IDs and timestamps.
  - Run selected observation through existing service, receipt, thesis, mandate, evaluator, and PaperOrderWriter boundaries.
  - Record one simulated FILLED result and one deterministic edge-collapse refusal.
  - Run full local gates and exact-head independent review.
tests_required:
  - native session artifact parse and monotonic update validation
  - script reproduction of recorded hashes/outcomes
  - npm run check
  - npm test
  - npm run build
  - npm run secret-scan
  - git diff --check
evidence_produced:
  - input observation count: 5
  - evaluation count: 2
  - successful path: EXECUTION_INTENT -> PAPER FILLED
  - refusal path: EXECUTABLE_EDGE_TOO_LOW
  ...
  - native source update IDs strictly monotonic
  - implementation candidate: c45f575
  - final exact-head review: APPROVE, safe_to_integrate=true at c45f575
evidence_ceiling: >
  Bounded native Binance public bookTicker observations plus deterministic local
  top-of-book PAPER simulation. No synchronized depth, private account truth,
  exchange write, profitability, or production readiness claim.
