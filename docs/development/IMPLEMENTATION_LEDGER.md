...
| M-B5 | OrderWriter + Reconciliation | REMEDIATION_COMPLETE / LOCAL_PASS — exact candidate reported by remediation receipt; FILLED requires cumulative quantity >= requested; cancellation persists REQUESTED before adapter and durable UNKNOWN on uncertainty; deterministic ownership binds client ID, mandate/workflow/attempt, and intent fingerprint; focused `10/10`, full `410/410`, check/build/diff-check PASS; no live execution or production durability claimed |
...
- status: `REMEDIATION_COMPLETE / LOCAL_PASS`; candidate implements canonical own-data validation for intents/events, complete intent fingerprint idempotency, consume/persist-before-adapter ordering, immutable audit-bound receipts, injected local/replay adapter, timeout-to-`UNKNOWN` with no blind retry, FILLED cumulative-quantity consistency, monotonic duplicate-safe reconciliation with out-of-order protection and weighted average price, and durable writer-owned cancellation states.
- TDD: new FILLED, durable cancel REQUESTED/UNKNOWN, and forged ownership regressions RED before implementation, then GREEN; focused execution `10/10`; full suite `410/410`; typecheck/build/diff-check `PASS`.
