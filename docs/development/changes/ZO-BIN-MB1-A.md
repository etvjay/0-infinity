...
- Full-suite and build receipts for this candidate are recorded below; evidence ceiling remains `LOCAL_PASS`. The integrated remediation checkpoint is `M-B1-AUTHORITY-VALIDATION`; current canonical verification is `npm test` — 296/296, domain — 11/11, and the current kernel remediation remains pending independent review.
