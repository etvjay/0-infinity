...
export function validMandate(value: unknown): value is ExecutionMandate {
  try {
  const mandateKeys = ["mandateId", "workflowId", "thesisId", "thesisHash", "method", "advocateRef", "opposeRef", "marketAnalysisRef", "evidenceBundleHash", "councilDecisionHash", "reasoningReceiptHash", "provenance", "venue", "instrument", "symbol", "side", "accountId", "expiresAt", "validity", "anchor", "entry", "economics", "risk", "invalidation", "execution", "version", "maxUses"];
  if (!isCanonicalFrozenObject(value, mandateKeys) || !frozenTree(value) || value.version !== 1 || value.maxUses !== 1) return false;
  ...
  if (strings.some((v) => !text(v)) || (x.thesisHash !== undefined && !text(x.thesisHash)) || !text(x.reasoningReceiptHash) || x.venue !== "BINANCE" || !["SPOT", "USD_M_FUTURES"].includes(String(x.instrument)) || !["BUY", "SELL"].includes(String(x.side))) return false;
