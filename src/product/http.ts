...
export const createHttpServer = (service = new ZeroInfinityService()) => createServer((request, response) => { if (request.headers["content-type"] && !request.headers["content-type"].startsWith("application/json")) { response.statusCode = 415; response.end(JSON.stringify({ error: "UNSUPPORTED_MEDIA_TYPE" })); return; } let raw = ""; request.on("data", chunk => { raw += chunk; if (raw.length > MAX_JSON) request.destroy(); }).on("end", async () => { let body: unknown; try { body = raw ? JSON.parse(raw) : undefined; } catch { response.statusCode = 400; response.end(JSON.stringify({ error: "INVALID_JSON" })); return; } const result = await handleRequest(service, request.method ?? "GET", new URL(request.url ?? "/", "http://localhost").pathname, body); response.statusCode = result.status; response.setHeader("content-type", "application/json"); response.end(JSON.stringify(result.body)); }); });
