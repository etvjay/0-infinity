...
test("product receipt provenance is bound to the published thesis", async () => {
  const service = new ZeroInfinityService({ clock: () => 1_700_000_000_000, idFactory: () => "wf-provenance" });
  const workflow = service.createWorkflow({ symbol: "BTCUSDT", venue: "BINANCE", product: "USD_M_FUTURES" });
  const submitted = await service.submitOpportunity(workflow.workflowId);
  assert.equal(submitted.workflow.status, "COMPLETE");
  const thesis = submitted.thesis as any;
  const receipt = service.getReasoningReceipt(workflow.workflowId)!;
  assert.equal(thesis.reasoning.reasoningReceiptHash, receipt.canonicalSha256);
  assert.equal(receipt.workflowId, workflow.workflowId);
  assert.equal(receipt.opportunity.symbol, thesis.symbol);
...
test("paper execution is visible from the canonical workflow read model", async () => {
  ...
  await service.runPaperLiveWorkflow({ symbol: "BTCUSDT", venue: "BINANCE", product: "USD_M_FUTURES", paperExecution: {
  ...
  const read = service.getWorkflow("wf-read-model") as any;
  assert.equal(read.execution.status, "FILLED");
