(() => {
  ...
  async function refresh() {
    if ($('demo-toggle').checked) { setState(false, demo); $('receipt-hash').textContent = 'demo fixture has no receipt hash'; return; }
    if (!apiBase) { setState(false, null); $('receipt-hash').textContent = 'no API configured'; return; }
    try {
    ...
    } catch (error) { setState(false, null); $('receipt-hash').textContent = 'API did not return a receipt'; }
  }
